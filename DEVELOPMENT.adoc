...
* **bump.yml** — Triggered manually from the Actions tab. Calls `rust-bump-version` to run CI, bump the version, and open a pull request.
* **tag.yml** — Fires when GitHub merges a pull request whose head branch starts with `bump-v`. Calls `rust-tag-on-merge` to create and push the annotated tag, which then triggers `deploy.yml`.
...
You can release a new version either:

* In GitHub Actions (the *Bump version* workflow)
* Locally with the `just bump` script and a PR
